...
public:
 ...
 Hive_Structure(nnx value_size)
  :  block_package_(nullptr), layer_size_(0), value_size_(value_size), total_size_(0),
     default_out_of_bounds_resolution_flags_(Out_of_Bounds_Resolution_Flags::N_A)
 {
 }
 ...
 void preinit_all_layers(nx layer_size)
 {
  for(nnx i = 0; i < block_package_->block_size_; ++i)
  {
   Hive_Layer& layer = block_package_->first_block_->layers_[i];
   layer.elements_ = malloc(value_size_ * layer_size);
  }
 }

 void preinit_all_layers()
 {
  preinit_all_layers(layer_size());
 }
